function [I,I_grad,Waypoints] = factor_scaled_integral_univ(log_func,theta,prop,delta,L)
...
ints = zeros(d,1);
for i=1:d
    if L(i)/prop < theta_mod(i,1)  %numerical check that the integral is finite      
    ...
    %    u = [theta_mod(i,1), log(integral(@(t) exp(log_func(t)/prop -.5*theta_mod(i,1).*t.^2+theta_mod(i,2).*t),-inf,inf,'Waypoints',wp))]
        ints(i) = log(integral(@(t) exp(log_func{i}(t)/prop -.5*theta_mod(i,1).*t.^2+theta_mod(i,2).*t),-inf,inf,'Waypoints',[-wp 0 wp]));
    else   
        ints(i) = inf;
        break
    end
